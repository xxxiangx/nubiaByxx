                ...
                $.ajax({
                    ...
                    if ($username.val()) {
                        ...
                        if ($reg.test($username.val())) {
                            ...
                            // $aspan.eq(0).show().css('visibility', 'visible').html('√');
                            // telflag = true;
                        } else {
                            $aspan.eq(0).show().css('color', 'red').html('请输入11位有效中国手机号码！');
                            telflag = false;
                        }
                    } else {
                        $aspan.eq(0).show().css('color', 'red').html('请输入手机号码！');
                        telflag = false;
                    }
                });
